  ...
  return (
    <div className="flex w-[284px] items-center gap-2 pr-5 pl-3.5 py-3">
      ...
      <div className="flex items-baseline gap-1.5 text-[#F7A815] ml-auto pr-0.5">
        ...
        <Counter paused={isPaused} reset={shouldReset} />
      </div>
    </div>
  );
}

function Counter({ paused, reset }: { paused?: boolean, reset?: boolean }) {
      ...
      <AnimatePresence initial={false} mode="popLayout">
        {countArray.map((n, i) => (
          <motion.div
            ...
            key={n + i}
            initial={{ y: 12, filter: "blur(2)", opacity: 0 }}
            animate={{ y: 0, filter: "blur(0px)", opacity: 1 }}
            exit={{ y: -12, filter: "blur(2)", opacity: 0 }}
